...
text = re.sub(
    r'def _extract_localtunnel_url.*?return None\n\n\n',
    r'',
    text, flags=re.DOTALL
)
text = re.sub(
    r'def _find_localtunnel_url_from_log.*?return None\n\n\n',
    r'',
    text, flags=re.DOTALL
)
text = re.sub(
    r'def _stop_localtunnel_processes.*?\n\n\n',
    r'',
    text, flags=re.DOTALL
)
text = re.sub(
    r'def _start_localtunnel.*?return None\n\n\n',
    r'',
    text, flags=re.DOTALL
)
...
ngrok_func = """def _start_ngrok(port: int) -> str | None:
    try:
        ...
        return None\n\n"""
text = text.replace("def _get_windows_pids_on_port(", ngrok_func + "def _get_windows_pids_on_port(")
...
with open(r"d:\superbrain\backend\start_ngrok_last.py", "w", encoding="utf-8") as f:
    f.write(text)
print("done")
