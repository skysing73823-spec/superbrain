...
# 1. Add pyngrok
if '("pyngrok", "pyngrok")' not in text:
    text = re.sub(
...
text = text.replace("localtunnel_enabled", "NGROK_ENABLED.exists()")
text = text.replace('npx localtunnel --port {PORT}', 'ngrok http {PORT}')
text = text.replace('npx localtunnel --port 5000', 'ngrok http 5000')
text = text.replace('(localtunnel)', '(ngrok)')

text = text.replace('check localtunnel.log)', 'check ngrok.log)')
